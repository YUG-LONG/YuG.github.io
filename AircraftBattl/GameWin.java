...
代码中定义了一些变量用于游戏状态的管理、游戏对象的维护等，
包括窗口大小、游戏状态、时间计数、敌机数量、得分、等级、boos、道具数量等。
GameWin 类实现了paint方法用于绘制游戏窗口的内容，具体的绘制根据游戏状态的不同而不同，
分别绘制开始界面、游戏运行界面、游戏失败界面和游戏胜利界面。

游戏画面的绘制使用双缓存解决了画面闪烁问题。

createObject方法用于游戏中敌机、我方子弹、Boos、敌方子弹、道具、障碍的生成，
并控制Boos血量低于百分之六十追踪发射子弹，低于百分之三十进入暴走状态，发射的子弹数量翻倍。
